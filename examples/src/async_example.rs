...
pub async fn insert_user_db<'a, 'b>(
    ...
    ctx.lock().unwrap_or_else(|e| e.into_inner()).full_name = Some(full_name.to_string());
    Ok(full_name)
}
...
pub async fn remove_user_db(ctx: Arc<Mutex<Context>>) -> Result<(), &'static str> {
    ctx.lock().unwrap_or_else(|e| e.into_inner()).removed_user = true;
    // some rollback logic
    Ok(())
}
// ==================================== generates AddBonusSalary, in the IronSagaAsyn you can mix between sync and async commands freely=============
#[ironcmd(result)]
pub async fn add_bonus_salary<'a>(
    salary: &'a str,
    bonus: u32,
    ctx: Arc<Mutex<Context>>,
) -> anyhow::Result<u32> {
    ...
    ctx.lock().unwrap_or_else(|e| e.into_inner()).salary = Some(new_salary);
    Ok(new_salary)
}
#[ironcmd(result, recursive_rollback)]
pub async fn add_user_redis(ctx: Arc<Mutex<Context>>) -> Result<String, &'static str> {
    if let Some(s) = ctx.lock().unwrap_or_else(|e| e.into_inner()).salary
        && s > 5000
    {
...
pub async fn async_example() {
    ...
    assert!(
        !ctx.lock().unwrap_or_else(|e| e.into_inner()).removed_user,
        "the rollback didnt run yet."
    );
    let fname = "salah";
    let lname = "aldeen";
    let mut user_insertion = InsertUser::new(fname, lname, ctx.clone());
    let rollback = RemoveUserDb::new(ctx.clone());
    user_insertion.set_rollback(rollback); // adding a rollback for the insertion command
    ...
    bus.add_command(bonus);
    bus.add_command(rds_user);
    ...
    //-----------------------
    assert_eq!(
        ctx.lock().unwrap_or_else(|e| e.into_inner()).full_name,
        Some("salah aldeen".into())
    );
    assert!(
        ctx.lock().unwrap_or_else(|e| e.into_inner()).removed_user,
    ...
    assert_eq!(
        ctx.lock().unwrap_or_else(|e| e.into_inner()).salary,
        Some(5099)
    );
}
